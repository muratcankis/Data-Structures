...
int lookup(char *anahtar, struct CELL *l){
    if(l==NULL) return 0;
    else if(!strcmp(anahtar,l->anahtar)) return 1;//strcmp iki tane stringi kıyaslıyor  içindekilerin birbiribne eşit olup olmamasını kontrol.
    else return lookup(anahtar,l->next);//0 döndürdüğünde yoksa o düğüm yerleştirmeye izin vericek
}

int insert(char *anahtar, struct CELL **l){//liste başı değişebilir **sebebi bu..
    if(*l==NULL){
        *l=(struct CELL*)malloc(sizeof(struct CELL));//yer ayırdık.//anahtar yoksa yerleştiriyor
        (*l)->anahtar=(char*)malloc((strlen(anahtar)+1)*sizeof(char));//uzunluğunun bir fazlası olmasının sebebi stringten gelen /0 için
        strcpy((*l)->anahtar,anahtar);
        (*l)->next=NULL;
        return 1;
    }
    else if(strcmp(anahtar,(*l)->anahtar)) //bu anahtarı yerleştireceğimiz anahtar ile kontrol ediyoruz
        return insert(anahtar, &((*l)->next));//listenin en sonuna yerleştiriyor
    else return 0;
}
